...
namespace EcommereceWeb.Infrstraction.EntityConfigration
{
    public class CouponItemConfigration : IEntityTypeConfiguration<CouponItem>
    {
        public void Configure(EntityTypeBuilder<CouponItem> builder)
        {
            ...
            builder.HasOne(a => a.Products).WithMany(b => b.CouponItems).HasForeignKey(b => b.ProductId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_MainClassification_BasicClassification");
            builder.HasOne(a => a.Coupons).WithMany(b => b.CouponItems).HasForeignKey(b => b.CouponId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_MainClassification_BasicClassification");
